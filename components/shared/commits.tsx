import React from "react";

const Commits = () => {
	return (
		<>
			<div className="top-0 z-1 grid grid-cols-12 grid-rows-12 gap-1">
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#161b22]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#006d32]"></div>
				<div className="size-4 rounded-sm bg-[#0e4429]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
				<div className="size-4 rounded-sm bg-[#39d353]"></div>
			</div>
			<div className="flex flex-row flex-wrap gap-x-6 sm:gap-x-4 md:gap-x-6 absolute bottom-1 p-2">
				<div>
					<span className="mr-1 text-sm text-zinc-600 dark:text-zinc-400">
						Stars:
					</span>
					242
				</div>
				<div>
					<span className="mr-1 text-sm text-zinc-600 dark:text-zinc-400">
						Followers:
					</span>
					70
				</div>
				<div>
					<span className="mr-1 text-sm text-zinc-600 dark:text-zinc-400">
						PRS:{" "}
					</span>
					64
				</div>
				<div>
					<span className="mr-1 text-sm text-zinc-600 dark:text-zinc-400">
						Issues:{" "}
					</span>
					17
				</div>
			</div>
		</>
	);
};

export default Commits;
